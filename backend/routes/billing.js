...
export async function handleBillingRoutes(request,response,{pathName,url}){
 ...
 const sendMatch=pathName.match(/^\/api\/invoices\/([^/]+)\/send$/);
 if(request.method==='POST'&&sendMatch){const context=await requireEmployee(request,response);if(!context)return true;const invoice=await runStoreTransaction({prisma:client=>sendInvoice(client,context,sendMatch[1])});sendJson(response,200,{invoice});return true}
 ...
 const paidMatch=pathName.match(/^\/api\/manager\/invoices\/([^/]+)\/paid$/);
 if(request.method==='POST'&&paidMatch){const context=await requireManager(request,response);if(!context)return true;const invoice=await runStoreTransaction({prisma:client=>markInvoicePaid(client,context,paidMatch[1])});sendJson(response,200,{invoice});return true}
 return false;
}
