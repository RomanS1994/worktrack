...
function monthRange(raw) {
  ...
  return { value, year, start, end, endExclusive };
}

function profileTax(user) {
  const profile = user?.profile && typeof user.profile === 'object' && !Array.isArray(user.profile) ? user.profile : {};
  const tax = profile.taxInformation && typeof profile.taxInformation === 'object' ? profile.taxInformation : {};
  return {
...
export async function createInvoiceDraft(client, context, payload = {}) {
  ...
  const entries = await client.workEntry.findMany({
    where: {
      companyId: membership.companyId, employeeMembershipId: membership.id, status: 'APPROVED',
      workDate: { gte: range.start, lt: range.endExclusive },
      invoiceItems: { none: { invoice: { status: { not: 'CANCELLED' } } } },
    }, include: { project: true }, orderBy: [{ workDate:'asc' }, { createdAt:'asc' }],
  });
  ...
  const invoice = await client.invoice.create({
    data: {
      ...
      sellerSnapshot: { ...tax, email: user.email, phone: user.phone || '' },
      buyerSnapshot: { name: company.name, companyId: company.id },
...
export async function listEmployeeInvoices(client, context) {
  const membership = employeeMembership(context);
  const invoices = await client.invoice.findMany({ where: { companyId: membership.companyId, employeeMembershipId: membership.id }, include: { items: { orderBy:{workDate:'asc'} } }, orderBy: { createdAt:'desc' } });
...
export async function sendInvoice(client, context, invoiceId) {
  ...
  const updated = await client.invoice.update({ where:{id:invoice.id}, data:{status:'SENT',sentAt:new Date()}, include:{items:{orderBy:{workDate:'asc'}}} });
...
export async function markInvoicePaid(client, context, invoiceId) {
  ...
  const updated = await client.invoice.update({where:{id:invoice.id},data:{status:'PAID',paidAt:new Date()},include:{items:{orderBy:{workDate:'asc'}}}});
  return serialize(updated);
}
