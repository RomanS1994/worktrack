...
import { useGetEmployeeAdvancesQuery, useGetManagerPayrollQuery, useGetWeekEntriesQuery, useGetWorkSummaryQuery } from '../../features/worktrack/worktrackApi.js';
...
const RATE_UNITS = { uk:'Kč/год',cs:'Kč/h',en:'CZK/h' };
...
function formatManagerRate(employee, locale, language) {
  if (employee.mixedRates || employee.mixedCustomerRates) return MIXED_RATE_LABELS[language] || MIXED_RATE_LABELS.uk;
  ...
  return `${formatCzk(payRate, locale)} → ${formatCzk(customerRate, locale)} ${RATE_UNITS[language] || RATE_UNITS.uk}`;
}
...
export function PayrollReportPage() {
  ...
  const weekEntriesQuery = useGetWeekEntriesQuery({weekStart:employeeAnchor},{skip:isManager||employeePeriod!=='week'});
  ...
  const error = activeQuery.error || (!isManager&&employeePeriod==='week'?weekEntriesQuery.error:null);
  const isLoading = activeQuery.isLoading || activeQuery.isFetching || (!isManager&&employeePeriod==='week'&&(weekEntriesQuery.isLoading||weekEntriesQuery.isFetching));
