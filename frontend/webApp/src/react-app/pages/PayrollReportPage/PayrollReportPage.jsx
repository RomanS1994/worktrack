...
export function PayrollReportPage() {
  ...
  return <section className="payrollReportPage pageStack">
    ...
    {!isLoading&&!error&&!isManager?<EmployeeFinanceDashboard companyName={companyName} hourlyRate={hourlyRate} language={language} locale={locale} localizedStatus={localizedStatus} monthData={employeePeriod==='month'?data:null} onChangePeriod={direction=>setEmployeeAnchor(current=>shiftAnchor(current,employeePeriod,direction))} onPeriodChange={next=>{setEmployeePeriod(next);setEmployeeAnchor(current=>next==='month'?`${monthKeyFromAnchor(current)}-01`:current)}} period={employeePeriod} submission={submission} summary={summary} user={user} week={week} workRules={data?.workRules}/>:null}
