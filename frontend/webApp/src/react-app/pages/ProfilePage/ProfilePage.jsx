...
const DETAIL_SECTIONS = ['personal', 'work', 'language', 'password', 'account'];
...
export function ProfilePage() {
  ...
  const roleLabel = membership?.role === 'MANAGER' ? t('profile.manager') : isEmployee ? t('profile.employee') : '-';
  const rate = membership?.hourlyRateCzk || '';
  const requestedSection = searchParams.get('section') || '';
  const section = DETAIL_SECTIONS.includes(requestedSection) ? requestedSection : '';
  const fromSettings = searchParams.get('from') === 'settings';
  const backTo = fromSettings ? '/more' : '/profile';
  ...
  const [avatarDraft, setAvatarDraft] = useState(user?.profile?.avatarDataUrl || '');
  ...
  async function handlePasswordSubmit(event) {
    ...
    try {
      const updatedUser = await changePassword({ currentPassword, newPassword }).unwrap();
      applyUpdatedUser(updatedUser);
      setCurrentPassword(''); setNewPassword(''); setConfirmPassword('');
      navigate(backTo);
    } catch (error) { setPasswordError(getApiErrorMessage(error)); }
  }
  ...
  const detailTitle = section === 'personal' ? t('profileUi.editPersonal') : section === 'work' ? t('profileUi.work') : section === 'language' ? t('profileUi.language') : section === 'password' ? t('profileUi.changePassword') : t('profileUi.danger');

  if (section) {
    return <section className="profilePage profileDetailPage pageStack">
      <header className="profileDetailHeader appTop">
        <BackButton to={backTo} />
        <div className="appTitleBlock"><h1>{detailTitle}</h1></div>
      </header>
      <section className="profileDetailCard screenCard">
        {section === 'personal' ? <form onSubmit={handleProfileSubmit} className="profileDetailBody">
          ...
          <div className="profileFieldGrid"><label className="profileField"><span>{t('profile.firstName')}</span><input type="text" value={firstName} onChange={e => setFirstName(e.target.value)} required /></label><label className="profileField"><span>{t('profile.lastName')}</span><input type="text" value={lastName} onChange={e => setLastName(e.target.value)} /></label></div>
          <label className="profileField"><span>{t('profile.phone')}</span><input type="tel" autoComplete="tel" placeholder="+420 777 123 456" value={phone} onChange={e => setPhone(e.target.value)} /></label>
          {profileError ? <p className="statusNote is-error">{profileError}</p> : null}
          <button className="profilePrimaryButton" disabled={profileState.isLoading}>{profileState.isLoading ? t('profile.saving') : t('profileUi.save')}</button>
        </form> : null}
  ...
  return <section className="profilePage pageStack">
    ...
    {user?.mustChangePassword ? <button className="profileNotice" type="button" onClick={() => openSection('password')}><strong>{t('profile.changeTemporaryPassword')}</strong><span>{t('profile.temporaryPasswordCopy')}</span><b>›</b></button> : null}
    <div className="profileGroups">
      <section><h2>{t('profileUi.account')}</h2><div className="profileMenu screenCard"><button type="button" onClick={() => openSection('personal')}><span className="profileMenuIcon">👤</span><span><strong>{t('profileUi.personal')}</strong><small>{t('profileUi.personalCopy')}</small></span><b>›</b></button><button type="button" onClick={() => openSection('work')}><span className="profileMenuIcon">💼</span><span><strong>{t('profileUi.work')}</strong><small>{t('profileUi.workCopy')}</small></span><b>›</b></button></div></section>
      <section><h2>{t('profileUi.preferences')}</h2><div className="profileMenu screenCard"><button type="button" onClick={() => openSection('language')}><span className="profileMenuIcon">🌐</span><span><strong>{t('profileUi.language')}</strong><small>{languageName}</small></span><b>›</b></button><button type="button" onClick={() => openSection('password')}><span className="profileMenuIcon">🔒</span><span><strong>{t('profileUi.security')}</strong><small>{t('profileUi.securityCopy')}</small></span><b>›</b></button>{isEmployee ? <button type="button" onClick={() => navigate('/tax-information')}><span className="profileMenuIcon">🧾</span><span><strong>{t('profileUi.tax')}</strong><small>{t('profileUi.taxCopy')}</small></span><b>›</b></button> : null}</div></section>
    </div>
    ...
    <button className="profileAccountLink" type="button" onClick={() => openSection('account')}>{t('profileUi.danger')}</button>
  </section>;
}
