...
export function TaxInformationPage(){
 ...
 const {data:monthData}=useGetMonthlyHoursQuery(currentMonth());
 ...
 const approvedHours=monthData?.summary?.approvedHours||'0.00';
 const approvedAmount=monthData?.summary?.approvedAmountCzk||'0.00';
 return <section className="taxPage pageStack">
  ...
  <section className="taxCard screenCard"><div className="compactHeader"><h2>{t('tax.invoicing')}</h2><p>{t('tax.invoiceCopy')}</p></div>
   <div className="invoicePreview"><div><span>{t('tax.month')}</span><strong>{currentMonthLabel(language)}</strong></div><div><span>{t('tax.approved')}</span><strong>{approvedHours} h</strong></div><div><span>{t('tax.amount')}</span><strong>{Number(approvedAmount).toLocaleString()} CZK</strong></div><span className="invoiceBadge">{t('tax.draft')}</span></div>
