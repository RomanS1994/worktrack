...
export function TaxInformationPage(){
 ...
 return <section className="taxPage pageStack">
  ...
  <section className="taxCard screenCard"><div className="compactHeader"><h2>{t('invoicing')}</h2><p>{t('invoiceCopy')}</p></div>
   <div className="invoicePreview"><div><span>{t('month')}</span><strong>{currentMonthLabel(locale)}</strong></div><div><span>{t('approved')}</span><strong>—</strong></div><div><span>{t('amount')}</span><strong>— {form.currency}</strong></div><span className="invoiceBadge">{t('draft')}</span></div>
   <button className="taxPrimary" type="button" disabled>{t('create')}</button><p className="taxNote">{t('note')}</p>
  </section>
 </section>;
}
