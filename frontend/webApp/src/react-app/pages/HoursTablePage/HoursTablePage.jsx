...
const COPY={
 ...
 cs:{eyebrow:'Přehledy',title:'Tabulka hodin',subtitle:'Měsíční přehled pracovních hodin, stavů a částek.',total:'Celkem hodin',approved:'Schváleno',pending:'Čeká',approvedAmount:'Potvrzeno',pendingAmount:'Očekávaná částka',date:'Datum',project:'Projekt',hours:'Hodiny',status:'Stav',empty:'V tomto měsíci zatím nejsou žádné záznamy.',loading:'Načítání…',draft:'Koncept',submitted:'Odesláno',approvedStatus:'Schváleno',rejected:'Vráceno'},
...
export function HoursTablePage(){
 ...
 return <section className="hoursTablePage pageStack">
  ...
  <section className="hoursTableSummary">
   <article><span>{c.total}</span><strong>{summary.totalHours||'0.00'} h</strong></article><article><span>{c.approved}</span><strong>{summary.approvedHours||'0.00'} h</strong></article><article><span>{c.pending}</span><strong>{summary.pendingHours||'0.00'} h</strong></article><article><span>{c.approvedAmount}</span><strong>{money(summary.approvedAmountCzk,language)}</strong></article><article><span>{c.pendingAmount}</span><strong>{money(summary.pendingAmountCzk,language)}</strong></article>
  </section>
  <section className="hoursTableCard screenCard">{isFetching?<p className="statusNote">{c.loading}</p>:rows.length?<div className="hoursTableScroll"><table><thead><tr><th>{c.date}</th><th>{c.project}</th><th>{c.hours}</th><th>{c.status}</th></tr></thead><tbody>{rows.map(row=><tr key={row.id}><td>{dateLabel(row.date,language)}</td><td>{row.project||'—'}</td><td><strong>{row.hours} h</strong></td><td><span className={`hoursTableStatus is-${String(row.status).toLowerCase()}`}>{statusLabel(row.status,c)}</span></td></tr>)}</tbody></table></div>:<p className="hoursTableEmpty">{c.empty}</p>}</section>
 </section>;
}
