...
const EMPTY_EMPLOYEE_FORM = { firstName: '', lastName: '', email: '', temporaryPassword: '', hourlyRateCzk: '', customerRateCzk: '' };
...
export function EmployeesPage() {
  ...
  const numericCustomerRateDraft = Number(customerRateDraft);
  const hasValidRateDraft = rateDraft !== '' && Number.isFinite(numericRateDraft) && numericRateDraft >= 0;
  const hasValidCustomerRateDraft = customerRateDraft !== '' && Number.isFinite(numericCustomerRateDraft) && numericCustomerRateDraft >= 0;
  ...
  function openManage(employee){clearMessages();setSelectedEmployeeId(employee.id);setRateDraft(employee.hourlyRateCzk || '0.00');setCustomerRateDraft(employee.customerRateCzk || employee.hourlyRateCzk || '0.00');setResetPassword('');setModal('manage')}
  ...
  async function saveRate(){
    if(!selectedEmployee||!hasValidRateDraft||!hasValidCustomerRateDraft)return;
    clearMessages();
    try{
      await updateManagerEmployee({employeeId:selectedEmployee.id,hourlyRateCzk:rateDraft,customerRateCzk:customerRateDraft}).unwrap();
  ...
  return <section className="employeesPage pageStack">
    ...
    {modal?<div className="employeesModalBackdrop" onMouseDown={e=>{if(e.target===e.currentTarget)closeModal()}}><section className={`employeesModal${modal==='manage'?' is-drawer':''}`} role="dialog" aria-modal="true"><header><div><span>{t('employees.team')}</span><h2>{modalTitle}</h2></div><button type="button" aria-label={t('employees.close')} onClick={closeModal}>×</button></header>
      {modal==='add'?<form className="employeesModalBody" onSubmit={submitEmployee}><p className="employeesModalCopy">{t('employees.addCopy')}</p><div className="employeesFormGrid"><label className="employeesField"><span>{t('employees.firstName')}</span><input type="text" value={form.firstName} onChange={e=>updateForm('firstName',e.target.value)} required/></label><label className="employeesField"><span>{t('employees.lastName')}</span><input type="text" value={form.lastName} onChange={e=>updateForm('lastName',e.target.value)} required/></label></div><label className="employeesField"><span>{t('employees.email')}</span><input type="email" value={form.email} onChange={e=>updateForm('email',e.target.value)} required/></label><label className="employeesField"><span>{t('employees.temporaryPassword')}</span><input type="password" minLength={8} value={form.temporaryPassword} onChange={e=>updateForm('temporaryPassword',e.target.value)} required/></label><div className="employeesFormGrid"><label className="employeesField"><span>{t('employees.hourlyRate')}</span><small>{t('employees.hourlyRateHint')}</small><input inputMode="decimal" type="number" min="0" step="0.01" value={form.hourlyRateCzk} onChange={e=>updateForm('hourlyRateCzk',e.target.value)} required/></label><label className="employeesField"><span>{t('employees.customerRate')}</span><small>{t('employees.customerRateHint')}</small><input inputMode="decimal" type="number" min="0" step="0.01" value={form.customerRateCzk} onChange={e=>updateForm('customerRateCzk',e.target.value)}/></label></div>{actionError?<p className="statusNote is-error">{actionError}</p>:null}<button className="employeesPrimaryButton" disabled={isMutating}>{t('employees.addEmployee')}</button></form>:null}
      {modal==='manage'&&selectedEmployee?<div className="employeesModalBody"><section className="employeeDetailHero"><span className={`employeeDetailAvatar${getEmployeeAvatar(selectedEmployee)?' has-photo':''}`}>{getEmployeeAvatar(selectedEmployee)?<img src={getEmployeeAvatar(selectedEmployee)} alt=""/>:getEmployeeName(selectedEmployee,t('employees.employee')).slice(0,1).toUpperCase()}</span><div><strong>{getEmployeeName(selectedEmployee,t('employees.employee'))}</strong><p>{selectedEmployee.email}</p></div><em className={selectedEmployee.status==='ACTIVE'?'is-active':''}>{selectedEmployee.status==='ACTIVE'?t('employees.activeStatus'):t('employees.inactiveStatus')}</em></section><section className="employeeDetailStats"><div><strong>{formatHours(selectedEmployee.summary?.totalHours)} h</strong><span>{t('employees.week')}</span></div><div><strong>{selectedEmployee.pendingSubmissions||0}</strong><span>{t('employees.pending')}</span></div></section><section className="employeeDetailSection"><h3>{t('employees.employeeDetails')}</h3><div className="employeesFormGrid"><label className="employeesField"><span>{t('employees.rate')}</span><small>{t('employees.hourlyRateHint')}</small><input inputMode="decimal" type="number" min="0" step="0.01" value={rateDraft} onChange={e=>setRateDraft(e.target.value)}/></label><label className="employeesField"><span>{t('employees.customerRate')}</span><small>{t('employees.customerRateHint')}</small><input inputMode="decimal" type="number" min="0" step="0.01" value={customerRateDraft} onChange={e=>setCustomerRateDraft(e.target.value)}/></label></div><button className="employeesPrimaryButton" type="button" disabled={isMutating||!hasValidRateDraft||!hasValidCustomerRateDraft} onClick={saveRate}>{t('employees.saveRate')}</button></section><section className="employeeDetailSection"><h3>{ac.access}</h3><div className="employeeAction"><span>{ac.employeeCabinet}</span><span>{ac.alwaysEnabled}</span></div><button className="employeeAction" type="button" disabled={isMutating} onClick={toggleManagerAccess}><span>{ac.managerCabinet}<small>{Boolean(selectedEmployee.canAccessManagerCabinet ?? selectedEmployee.role==='MANAGER')?ac.enabled:ac.disabled}</small></span><strong>{Boolean(selectedEmployee.canAccessManagerCabinet ?? selectedEmployee.role==='MANAGER')?ac.revoke:ac.grant}</strong></button></section>{actionError?<p className="statusNote is-error">{actionError}</p>:null}{actionSuccess?<p className="statusNote is-success">{actionSuccess}</p>:null}<section className="employeeDetailSection"><h3>{t('employees.actions')}</h3><button className="employeeAction" type="button" onClick={()=>{setResetPassword('');setModal('password')}}>{t('employees.resetPassword')}<span>›</span></button><button className={`employeeAction ${selectedEmployee.status==='ACTIVE'?'is-danger':'is-success'}`} type="button" disabled={isMutating} onClick={toggleEmployeeStatus}>{selectedEmployee.status==='ACTIVE'?t('employees.deactivate'):t('employees.reactivate')}<span>›</span></button><button className="employeeAction is-danger" type="button" disabled={isMutating||Boolean(selectedEmployee.canAccessManagerCabinet ?? selectedEmployee.role==='MANAGER')} onClick={()=>{setActionError('');setModal('delete')}}>{t('employees.deleteEmployee')}<span>›</span></button></section></div>:null}
