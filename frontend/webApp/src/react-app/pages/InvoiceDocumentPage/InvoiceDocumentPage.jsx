...
export function InvoiceDocumentPage({managerMode=false}){
 ...
 const seller=invoice.seller||{};const buyer=invoice.buyer||{};const paymentQr=qrUrl(invoice.paymentDescriptor);const history=historyQuery.data?.history||[];const currentStatus=statusLabel(c,invoice);const mixedRates=hasMixedRates(invoice.items||[]);
 return <section className="invoiceDocumentPage">
  ...
  <article className={`invoicePaper${invoice.isOverdue?' is-overdue':''}`} data-invoice-number={invoice.invoiceNumber}>
   ...
   <section className="invoiceDates"><div><span>{c.issue}</span><strong>{invoice.issueDate}</strong></div><div className={invoice.isOverdue?'is-overdue':''}><span>{c.due}</span><strong>{invoice.dueDate}</strong></div>{invoice.status==='PAID'&&invoice.paidAt?<div className="is-paid"><span>{c.paidDate}</span><strong>{dateValue(invoice.paidAt)}</strong></div>:null}<div><span>{c.period}</span><strong>{invoice.periodStart} — {invoice.periodEnd}</strong></div></section>
   ...
   <footer className="invoiceTotal"><div><span>{invoice.totalHours} h</span><span>{mixedRates?c.mixedRates:`× ${money(invoice.hourlyRate,invoice.currency)}`}</span></div><div><span>{c.total}</span><strong>{money(invoice.subtotal,invoice.currency)}</strong></div></footer>
  </article>
