...
export function InvoiceDocumentPage({managerMode=false}){
 ...
 async function copyPayment(){
  if(!invoice?.paymentDescriptor)return;
  try{await navigator.clipboard.writeText(invoice.paymentDescriptor);setCopied(true);window.setTimeout(()=>setCopied(false),1800)}catch{/* Clipboard may be unavailable on some embedded browsers. */}
 }
