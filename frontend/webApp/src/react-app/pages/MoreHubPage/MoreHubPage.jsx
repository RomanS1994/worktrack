...
const COPY = {
  uk: { title:'Налаштування',copy:'Керуйте параметрами компанії та акаунта.',changePhoto:'Змінити фото',photoError:'Не вдалося змінити фото.',photoTooLarge:'Фото має бути до 5 МБ.',companyGroup:'Компанія',identity:'Дані компанії',identityCopy:'Назва, адреса та системні дані',billing:'Реквізити',billingCopy:'IČO, DIČ, адреса та email для рахунків',workGroup:'Робочі правила',work:'Робочі правила',workCopy:'Обід, денна норма та графік роботи',personal:'Особисті дані',accountGroup:'Акаунт',language:'Мова',languageCopy:'Мова інтерфейсу застосунку',security:'Змінити пароль',securityCopy:'Оновіть пароль для входу в акаунт',deleteAccount:'Видалити акаунт',deleteAccountCopy:'Назавжди видалити акаунт і доступ',signOut:'Вийти з акаунта',signingOut:'Вихід…',manager:'Менеджер',employee:'Працівник' },
  cs: { title:'Nastavení',copy:'Spravujte nastavení společnosti a účtu.',changePhoto:'Změnit fotografii',photoError:'Fotografii se nepodařilo změnit.',photoTooLarge:'Fotografie může mít nejvýše 5 MB.',companyGroup:'Společnost',identity:'Údaje společnosti',identityCopy:'Název, adresa a systémové údaje',billing:'Fakturační údaje',billingCopy:'IČO, DIČ, adresa a e-mail pro faktury',workGroup:'Pracovní pravidla',work:'Pracovní pravidla',workCopy:'Přestávka, denní norma a pracovní plán',personal:'Osobní údaje',accountGroup:'Účet',language:'Jazyk',languageCopy:'Jazyk rozhraní aplikace',security:'Změnit heslo',securityCopy:'Aktualizujte heslo pro přihlášení',deleteAccount:'Odstranit účet',deleteAccountCopy:'Trvale odstranit účet a přístup',signOut:'Odhlásit se',signingOut:'Odhlašování…',manager:'Manažer',employee:'Pracovník' },
  en: { title:'Settings',copy:'Manage company and account settings.',changePhoto:'Change photo',photoError:'Could not change photo.',photoTooLarge:'Photo must be under 5 MB.',companyGroup:'Company',identity:'Company details',identityCopy:'Name, address and system details',billing:'Billing details',billingCopy:'Company ID, VAT ID, address and invoice email',workGroup:'Work rules',work:'Work rules',workCopy:'Lunch, daily standard and work schedule',personal:'Personal details',accountGroup:'Account',language:'Language',languageCopy:'Application interface language',security:'Change password',securityCopy:'Update your account sign-in password',deleteAccount:'Delete account',deleteAccountCopy:'Permanently remove your account and access',signOut:'Sign out',signingOut:'Signing out…',manager:'Manager',employee:'Employee' },
};

function getName(user){return user?.name||[user?.firstName,user?.lastName].filter(Boolean).join(' ')||user?.email||'-'}
function getCompanyName(user){return user?.activeMembership?.company?.name||user?.company?.name||''}
function getRoleLabel(user,c){return user?.activeMembership?.role==='EMPLOYEE'?c.employee:c.manager}
...
export function MoreHubPage(){
 const dispatch=useDispatch();const navigate=useNavigate();const user=useSelector(selectUser);const token=useSelector(selectToken);const {language}=useI18n();const c=COPY[language]||COPY.uk;const photoInputRef=useRef(null);const [updateProfile,updateState]=useUpdateProfileMutation();const [logout,logoutState]=useLogoutMutation();const [photoError,setPhotoError]=useState('');const [logoutError,setLogoutError]=useState('');const avatar=user?.profile?.avatarDataUrl||'';const companyName=getCompanyName(user);const roleLabel=getRoleLabel(user,c);
 ...
 return <section className="moreHub pageStack">
  ...
  <section className="moreHubProfile screenCard">
   ...
   <Link className="moreHubProfileMain" to="/profile?section=personal&from=settings" aria-label={c.personal}><span className="moreHubProfileText"><strong>{getName(user)}</strong><small>{user?.email||''}</small><em>{[companyName,roleLabel].filter(Boolean).join(' · ')}</em></span><span className="moreHubProfileChevron" aria-hidden="true">›</span></Link>
   <button className="moreHubChangePhoto" type="button" onClick={()=>photoInputRef.current?.click()} disabled={updateState.isLoading}>{updateState.isLoading?'…':c.changePhoto}</button>
  ...
  <SettingsGroup title={c.companyGroup}><SettingsRow to="/company-settings?section=identity&from=settings" icon="building" tone="company" title={c.identity} copy={c.identityCopy}/><SettingsRow to="/company-settings?section=billing&from=settings" icon="receipt" tone="billing" title={c.billing} copy={c.billingCopy}/></SettingsGroup>
  ...
  <SettingsGroup title={c.accountGroup}><SettingsRow to="/profile?section=language&from=settings" icon="globe" tone="language" title={c.language} copy={c.languageCopy}/><SettingsRow to="/profile?section=password&from=settings" icon="lock" tone="security" title={c.security} copy={c.securityCopy}/></SettingsGroup>
  ...
  <div className="moreHubAccountActions">
   ...
   <Link className="moreHubDeleteAccount" to="/profile?section=account&from=settings"><SvgIcon name="trash"/><span><strong>{c.deleteAccount}</strong><small>{c.deleteAccountCopy}</small></span></Link>
  </div>
 </section>;
}
