...
export function InvoicesPage(){
 ...
 async function send(id){if(!window.confirm(c.sendConfirm))return;setError('');try{await sendInvoice(id).unwrap();await refetch()}catch(err){setError(getApiErrorMessage(err))}}
 async function cancel(id){if(!window.confirm(c.cancelConfirm))return;setError('');try{await cancelInvoice(id).unwrap();await refetch()}catch(err){setError(getApiErrorMessage(err))}}
