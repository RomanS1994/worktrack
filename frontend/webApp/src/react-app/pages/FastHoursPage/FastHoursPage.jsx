...
export function FastHoursPage(){
 const {language}=useI18n();const c=COPY[language]||COPY.uk;const navigate=useNavigate();const current=useMemo(weekStartNow,[]);const [weekStart,setWeekStart]=useState(current);
 ...
 const statusText=c[status.toLowerCase()]||status;
 const setHour=(date,value)=>{setMessage('');setActionError('');setDraft(v=>({...v,[date]:value}))};
 const weekdayPreset=()=>setDraft(Object.fromEntries(days.map((day,i)=>[day.date,i<5?'8':''])));
 const clear=()=>setDraft(Object.fromEntries(days.map(day=>[day.date,''])));
 async function save(){if(!selectedProjectId||locked)return;setMessage('');setActionError('');try{for(const day of days){const value=String(values[day.date]??'').trim();const hours=Number(value)||0;const existing=entries.find(e=>e.workDate===day.date&&e.projectId===selectedProjectId);if(existing&&hours<=0)await deleteEntry(existing.id).unwrap();else if(existing)await updateEntry({entryId:existing.id,hours:String(hours),projectId:selectedProjectId}).unwrap();else if(hours>0)await createEntry({workDate:day.date,hours:String(hours),projectId:selectedProjectId}).unwrap()}setDraft({});setMessage(c.saved)}catch(err){setActionError(getApiErrorMessage(err))}}
 async function send(){try{await save();await submitWeek({weekStart}).unwrap()}catch(err){setActionError(getApiErrorMessage(err))}}
 return <section className="fastHoursPage pageStack">
  ...
  <button className="fastSend" type="button" disabled={locked||busy||!entries.length} onClick={send}>{c.send} →</button>
