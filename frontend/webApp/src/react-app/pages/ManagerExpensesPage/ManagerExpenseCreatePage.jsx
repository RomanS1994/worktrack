...
const COPY={
 uk:{title:'Нова витрата',subtitle:'Додайте витрату компанії',employee:'Працівник',chooseEmployee:'Оберіть працівника',category:'Категорія',amount:'Сума',date:'Дата',note:'Примітка',optional:'необов’язково',save:'Зберегти витрату',saving:'Збереження…',notePlaceholder:'Наприклад: купівля матеріалів'},
 cs:{title:'Nový výdaj',subtitle:'Přidejte firemní výdaj',employee:'Zaměstnanec',chooseEmployee:'Vyberte zaměstnance',category:'Kategorie',amount:'Částka',date:'Datum',note:'Poznámka',optional:'nepovinné',save:'Uložit výdaj',saving:'Ukládání…',notePlaceholder:'Např. nákup materiálu'},
 en:{title:'New expense',subtitle:'Add a company expense',employee:'Employee',chooseEmployee:'Choose employee',category:'Category',amount:'Amount',date:'Date',note:'Note',optional:'optional',save:'Save expense',saving:'Saving…',notePlaceholder:'For example: materials purchase'}
};
...
export function ManagerExpenseCreatePage(){
 ...
 const canSubmit=employeeMembershipId&&Number(String(amountCzk).replace(',','.'))>0&&spentAt&&!state.isLoading;
 async function submit(e){
  e.preventDefault();
  if(!canSubmit)return;
  await createExpense({employeeMembershipId,category,amountCzk:String(amountCzk).replace(',','.'),spentAt,note}).unwrap();
