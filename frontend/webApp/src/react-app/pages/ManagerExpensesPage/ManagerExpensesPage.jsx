...
import { openExpenseReceipt } from '../../features/worktrack/expenseReceipt.js';
...
const COPY={
 uk:{title:'Витрати',subtitle:'Витрати компанії за вибраний період',month:'Місяць',category:'Категорія',all:'Усі категорії',total:'Витрачено за місяць',count:'Кількість витрат',history:'Історія витрат',empty:'За цей місяць витрат немає',add:'Додати витрату',delete:'Видалити',confirm:'Видалити цю витрату?',employee:'Працівник',receipt:'Чек',openingReceipt:'Відкриття…',receiptError:'Не вдалося відкрити фото чеку.'},
 cs:{title:'Výdaje',subtitle:'Firemní výdaje za zvolené období',month:'Měsíc',category:'Kategorie',all:'Všechny kategorie',total:'Výdaje za měsíc',count:'Počet výdajů',history:'Historie výdajů',empty:'V tomto měsíci nejsou žádné výdaje',add:'Přidat výdaj',delete:'Smazat',confirm:'Smazat tento výdaj?',employee:'Zaměstnanec',receipt:'Doklad',openingReceipt:'Otevírání…',receiptError:'Fotografii dokladu se nepodařilo otevřít.'},
 en:{title:'Expenses',subtitle:'Company expenses for the selected period',month:'Month',category:'Category',all:'All categories',total:'Spent this month',count:'Number of expenses',history:'Expense history',empty:'No expenses this month',add:'Add expense',delete:'Delete',confirm:'Delete this expense?',employee:'Employee',receipt:'Receipt',openingReceipt:'Opening…',receiptError:'Could not open the receipt photo.'}
};
...
export function ManagerExpensesPage(){
 ...
 return <section className="managerExpensesPage pageStack">
  ...
  {error?<p className="statusNote is-error">{getApiErrorMessage(error)}</p>:null}{receiptError?<p className="statusNote is-error">{receiptError}</p>:null}{query.isLoading?<RequestLoadingState label={copy.history}/>:null}
  {!query.isLoading?<section className="managerExpenses-history"><div className="managerExpenses-title"><h2>{copy.history}</h2><span>{expenses.length}</span></div>{groups.length?groups.map(([date,items])=><div className="managerExpenses-day" key={date}><div className="managerExpenses-date">{new Intl.DateTimeFormat(locale,{day:'numeric',month:'long',year:'numeric',timeZone:'UTC'}).format(new Date(`${date}T00:00:00Z`))}</div>{items.map(item=><article className="managerExpenses-row" key={item.id}><div className="managerExpenses-icon">₭</div><div className="managerExpenses-text"><strong>{CATEGORIES[item.category]?.[language]||CATEGORIES[item.category]?.uk||item.category}</strong><span>{item.employee?.name?`${copy.employee}: ${item.employee.name}`:(item.note||'—')}</span>{item.employee?.name&&item.note?<span>{item.note}</span>:null}{item.hasReceipt?<button type="button" className="managerExpenses-receiptButton" disabled={receiptLoadingId===item.id} onClick={()=>viewReceipt(item.id)}><span aria-hidden="true">▣</span>{receiptLoadingId===item.id?copy.openingReceipt:copy.receipt}</button>:null}</div><div className="managerExpenses-amount"><strong>− {formatCzk(item.amountCzk,locale)}</strong><button type="button" disabled={deleteState.isLoading} onClick={()=>remove(item.id)}>{copy.delete}</button></div></article>)}</div>):<div className="managerExpenses-empty">{copy.empty}</div>}</section>:null}
 </section>
}
