...
import { useCreateProjectMutation, useGetProjectsQuery, useUpdateProjectMutation } from '../../features/worktrack/worktrackApi.js';
...
export function ProjectsPage() {
  ...
  const [mutatingProjectId, setMutatingProjectId] = useState('');
  const [modalOpen, setModalOpen] = useState(false);
  const [actionError, setActionError] = useState('');
  const projects = Array.isArray(data?.projects) ? data.projects : [];
  const activeProjectCount = projects.filter(project => project.isActive).length;
  const isFormMutating = createState.isLoading || (updateState.isLoading && !mutatingProjectId);

  function updateForm(field, value) { setForm(current => ({ ...current, [field]: value })); }
  function openCreate() { setEditingId(''); setForm(EMPTY_FORM); setActionError(''); setModalOpen(true); }
  function startEdit(project) { setEditingId(project.id); setForm({ name: project.name || '', address: project.address || '', description: project.description || '' }); setActionError(''); setModalOpen(true); }
  ...
  return <section className="projectsPage pageStack">
    ...
    {actionError && !modalOpen ? <p className="statusNote is-error">{actionError}</p> : null}
